...
import ttsHuman
from entity.digitalHumanParam import LipsRealParam, DigitalHumanParam
...
async def tts_get_endpoint(
    ...
    if gpt_path_str is not None:
        tts_pipeline.init_t2s_weights('GPT_weights_v2/'+gpt_path_str)
    if sovits_path_str is not None:
        tts_pipeline.init_vits_weights('SoVITS_weights_v2/'+sovits_path_str)
    req = {
        "text": text,
...
async def tts_post_endpoint(request: TTS_Request):
    if request.gpt_path_str is not None:
        tts_pipeline.init_t2s_weights('GPT_weights_v2/' + request.gpt_path_str)
    if request.sovits_path_str is not None:
        tts_pipeline.init_vits_weights('SoVITS_weights_v2/' + request.sovits_path_str)
    req = request.dict()
    return await tts_handle(req)
...
def generateLipAndWavFile(params: LipsRealParam):
    print('收到的参数：', params)
    result = get_voice_template(params.id, params.msg)
...
def get_voice_template(id, msg):
    sql = 'select voice_name,model_code,rate,volume,pitch from tb_digital_voice_template where deleted=0 and status=1 and id = %s limit 1'
    result = mysqlHelper.selectone(sql, [id])
    if result is None:
        return None
    else:
        return DigitalHumanParam(msg=msg, voiceName=result[0], id=id, rate=int(result[2]), volume=int(result[3]),
                                 pitch=int(result[4]))
    pass
