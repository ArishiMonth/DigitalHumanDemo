...
class YjCommon:
    ...
    def upload_file(file_path, chunk_size=5 * 1024 * 1024):
        ...
        with open(file_path, 'rb') as file:
            ...
            for offset in range(0, total):
                ...
                response = requests.post(config.UPLOAD_URL + "upload-api/chunkUpload", files={'file': chunk},
                                         params={'domainId': 1, 'dir': "video", 'chunk': chunk_number,
                                                 'chunkTotal': total,
                                                 'taskId': task_id, "fileName": file_name},
                                         headers=headers,timeout=30*1000)
    ...
    def download_file(url, local_path):
        parent_path = YjCommon.get_parent_path(local_path)
