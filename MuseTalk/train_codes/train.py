        ...
        for step, (ref_image, image, masked_image, masks, audio_feature) in enumerate(train_data_loader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.reconstruction:  # decode the image from the predicted latents
                    ...
                    loss_lip = F.l1_loss(image_pred_img.float(), image.float(),
                                         reduction="mean")  # the loss of the decoded images
                    loss_latents = F.l1_loss(image_pred.float(), latents.float(),
                                             reduction="mean")  # the loss of the latents
